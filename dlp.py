...
from sklearn.metrics import classification_report, confusion_matrix, pairwise
...
class DynamicLabelPropagation(object):
    ...
    def dynamic_label_prop(self):
        ...
        knn_mat = self.calc_knn_graph(n_neighbors=self.prop_config['n_neighbors'], raw_mat=transition_mat)
        ...
        for i_row in range(labeled_y_mat.shape[0]):
            labeled_y_mat[i_row, np.where(class_vec == self.data_y_train[i_row])[0]] = 1
        ...
        # format output
        dlp_model = FormatModel(dlp_param=self.prop_config, orig_sample_index=self.orig_sample_index,
    ...
    def calc_transition_mat(self):
        ...
        if 'rbf' == self.prop_config['kernel']:
            transition_mat = pairwise.rbf_kernel(X=scaled_x, Y=scaled_x, gamma=self.prop_config['gamma'])
        else:
    ...
    def calc_knn_graph(self, n_neighbors, raw_mat):
        sort_idx_mat = np.argsort(-raw_mat, axis=1)[:, :n_neighbors]
...
class FormatModel(object):
    def __init__(self, dlp_param, orig_sample_index, labeled_index, unlabeled_index, classes, transduction):
        self.dlp_param_ = dlp_param

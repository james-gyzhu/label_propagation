...
Label propagation using scikit_dlp algorithms
...
import pprint
from scikit_dlp import LabelPropagation, LabelSpreading
...
class LabelPropScikitDlp(object):
    ...
    def iter_label_prop(self):
        ...
        for i in range(self.prop_config['iter_num']):
            ...
            if 'LabelSpreading' == self.prop_config['prop_algo']:
                iter_lp_model = LabelSpreading(gamma=self.prop_config['gamma'], max_iter=self.prop_config['max_iter'])
            else:
                iter_lp_model = LabelPropagation(gamma=self.prop_config['gamma'], max_iter=self.prop_config['max_iter'])
...
def iter_label_propagation(data_x, data_y_train, data_y_true, param_dict=prop_config_defaults):
    lp_model_inst = LabelPropScikitDlp(data_x=data_x, data_y_train=data_y_train, data_y_true=data_y_true,
